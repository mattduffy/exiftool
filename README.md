...
#### Exiftool Now Uses Spawn
Images are getting really BIG now.  Issuing metadata extraction commands on multiple large images could consume more memory than was allocated by default to node's ```child_process.exec()``` function.  Previous versions of Exiftool provided the ```exiftool.setMaxBufferMultiplier()``` method to add more memory headroom.  This approach did not work on all occasions and could result in incomplete results.  Exiftool now uses node's ```child_process.spawn()``` function to stream the metadata command results back so there is no need to allocate more memory to the process.  
